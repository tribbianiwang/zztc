...
import com.yascn.smartpark.utils.DisplayUtil;
...
//    private static final int BOTTOM_BTN_HEIGHT = Constant.RECT_VIEW_BOTTOM;
    ...
    private static final String TIPS = "请将行驶证放入到方框中";
...
//        viewHeight = panelHeght +(int) DisplayUtil.dp2px(activity,TOP_BAR_HEIGHT + BOTTOM_BTN_HEIGHT);
        ...
        rectHeght = (int) (rectWidth * 60.0 / 86.0);
        // 相对于此view
        ...
        wordPaint.setTextSize(45);

        rect = new Rect(rectLeft, rectTop - 80, rectRight, rectTop - 10);
        FontMetricsInt fontMetrics = wordPaint.getFontMetricsInt();
        baseline = rectBottom+ (fontMetrics.bottom-fontMetrics.top);
//        baseline = rect.bottom + (rect.bottom - rect.top - fontMetrics.bottom + fontMetrics.top) / 2 - fontMetrics.top;
